...
let batch = JSON.parse(localStorage.getItem('zxbatchPayslips') || '[]');
...
if ($('siRequired').checked) {
  ...
  if (wageType === 'monthly') {
    siBase = monthly + locationPay + holidayPay;
  } else {
    siBase = (daily * days) + locationPay + holidayPay;
  }

  const cappedBase = Math.min(siBase, 15000);
  const si = Math.round(cappedBase * 0.05);
...
$('addBatchBtn').addEventListener('click', ()=>{
  ...
  const obj = {
    ...
    daysWorked: fmt(parseNum($('daysWorked').value)),
  ...
  localStorage.setItem('zxbatchPayslips', JSON.stringify(batch));
  renderBatch();
  $('payslipForm').reset();
  updateWageTypeFields();
  calculate();
...
$('exportBatchBtn')?.addEventListener('click', ()=> {
  ...
  link.download = 'ZHONGXINGAll_Payslips.csv';
  link.click();
});
...
$('exportIndividualBtn')?.addEventListener('click', ()=> {
  ...
  const link = document.createElement('a');
  link.href = URL.createObjectURL(blob);
  link.download = 'ZHONGXINGIndividual_Payslips.csv';
  link.click();
});
...
$('clearAllBtn')?.addEventListener('click', () => {
  ...
  if (confirm('Are you sure to clear all payslips?')) {
    batch = [];
    localStorage.removeItem('zxbatchPayslips');
    renderBatch();
