...
function calculate(){
  ...
  const telephone = parseNum($('telephoneBill').value);

  // Basic Pay
  const basicTotal = (wageType==='daily'? daily * days: (monthly / 2)) + telephone;
...
$('addBatchBtn').addEventListener('click', ()=>{
  ...
  const obj = {
    ...
    telephoneBill: $('telephoneBill').value || '0',
...
function renderBatch(displayBatch = batch){
  ...
  if(displayBatch.length === 0){
    el.textContent = 'No payslips added';
    return;
  }
